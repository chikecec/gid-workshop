      ...
      <div style={{ background: '#fff', borderBottom: '1px solid #eee', padding: '14px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          ...
          <div style={{ fontSize: '11px', color: '#999', marginTop: '1px' }}>
            {filtered.length} of {equipment.length} devices
          </div>
        </div>
        ...
        {!loading && filtered.length === 0 && (
          <div style={{ textAlign: 'center', padding: '40px', color: '#aaa' }}>
            ...
            <div style={{ fontSize: '12px' }}>
              {activeFilterCount > 0 || search ? 'Try adjusting your filters' : 'Tap + to register your first device'}
            </div>
            ...
            <div style={{ marginBottom: '16px' }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                {[
                  ...
                  <button key={s.key} onClick={() => togglePending('pmStatus', s.key)} style={chipStyle(pendingFilters.pmStatus.includes(s.key))}>
                    {s.label}
                  </button>
                ))}
              </div>
            ...
            <div style={{ marginBottom: '24px' }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                {[
                  ...
                  <button key={s.key} onClick={() => togglePending('opStatus', s.key)} style={chipStyle(pendingFilters.opStatus.includes(s.key))}>
                    {s.label}
                  </button>
                ))}
              </div>
