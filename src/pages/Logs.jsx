...
export default function Logs({ facility }) {
  ...
  const [filters, setFilters] = useState({ serviceTypes: [], statuses: [], billingTypes: [] })
  const [pendingFilters, setPendingFilters] = useState({ serviceTypes: [], statuses: [], billingTypes: [] })
    ...
    <div>
      <div style={{ background: '#fff', borderBottom: '1px solid #eee', padding: '14px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          ...
          <div style={{ fontSize: '11px', color: '#999', marginTop: '1px' }}>{filtered.length} of {logs.length} records</div>
        </div>
      ...
      <div style={{ padding: '10px 16px', display: 'flex', flexDirection: 'column', gap: '10px', paddingBottom: '100px' }}>
        ...
        {loading && <div style={{ textAlign: 'center', padding: '40px', color: '#aaa', fontSize: '13px' }}>Loading logs...</div>}
        ...
        {!loading && logs.length > 0 && filtered.length === 0 && (
          <div style={{ textAlign: 'center', padding: '40px', color: '#aaa' }}>
            ...
            <button onClick={clearFilters} style={{ marginTop: '8px', padding: '7px 16px', borderRadius: '8px', border: '1px solid #ddd', background: '#fff', fontSize: '12px', color: '#666', cursor: 'pointer' }}>Clear filters</button>
          </div>
        )}
                  ...
                  {(log.equipment?.model_number || log.equipment?.serial_number) && (
                    <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
                      ...
                      {log.equipment?.serial_number && (
                        <div style={{ fontSize: '11px', color: '#888' }}>
                          S/N: <span style={{ fontWeight: '500', color: '#444' }}>{log.equipment.serial_number}</span>
                        </div>
                      )}
                    </div>
                  )}
            ...
            <div style={{ marginBottom: '16px' }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                {Object.entries(typeConfig).map(([key, tc]) => (
                  <button key={key} onClick={() => togglePending('serviceTypes', key)} style={chipStyle(pendingFilters.serviceTypes.includes(key))}>{tc.label}</button>
                ))}
              </div>
            </div>
            ...
            <div style={{ marginBottom: '16px' }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                {Object.entries(statusLabels).map(([key, label]) => (
                  <button key={key} onClick={() => togglePending('statuses', key)} style={chipStyle(pendingFilters.statuses.includes(key))}>{label}</button>
                ))}
              </div>
            </div>
            ...
            <div style={{ marginBottom: '24px' }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                {Object.entries(billingLabels).map(([key, label]) => (
                  <button key={key} onClick={() => togglePending('billingTypes', key)} style={chipStyle(pendingFilters.billingTypes.includes(key))}>{label}</button>
                ))}
              </div>
            </div>
